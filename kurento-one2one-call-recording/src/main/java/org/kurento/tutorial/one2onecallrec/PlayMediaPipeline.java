...
import org.kurento.client.KurentoClient;
 ...
 public class PlayMediaPipeline {
   ...
   private HttpPostEndpoint hpEp;
   private String url;
 
   public PlayMediaPipeline(KurentoClient kurento, UserSession user, final WebSocketSession session, Boolean isComposite, String uri, Boolean isWebRtc) {
     // Media pipeline
     pipeline = kurento.createMediaPipeline();
 
     // Media Elements (WebRtcEndpoint, PlayerEndpoint)
     player = new PlayerEndpoint.Builder(pipeline, uri).build();
     
     // Use HttpPostEndpoint //shj
     hpEp = new HttpPostEndpoint.Builder(pipeline).build();
    //  player.connect(hpEp);
     url = hpEp.getUrl();
     
     // Connection // Use webRtcEndpoint
     webRtc = new WebRtcEndpoint.Builder(pipeline).build();
     if(isWebRtc){
       player.connect(webRtc, MediaType.AUDIO);
       player.connect(webRtc, MediaType.VIDEO);
     }
   ...
   public HttpPostEndpoint getHpEp(){
    return hpEp;
   }
